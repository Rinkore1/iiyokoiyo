...
public class FileDao {
	...
	public File getById(long id) {
		...
		if (!result.isEmpty()) {
			...
			file.setName(Bytes.toString(result.getValue(Bytes.toBytes(Constants.FAMILY_FILE_FILE),
					Bytes.toBytes(Constants.COLUMN_FILE_ORIGINALNAMEANDETC[1]))));
			file.setFile(Bytes.toBoolean(result.getValue(Bytes.toBytes(Constants.FAMILY_FILE_FILE),
					Bytes.toBytes(Constants.COLUMN_FILE_ORIGINALNAMEANDETC[2]))));
			file.setDir(Bytes.toBoolean(result.getValue(Bytes.toBytes(Constants.FAMILY_FILE_FILE),
					Bytes.toBytes(Constants.COLUMN_FILE_ORIGINALNAMEANDETC[3]))));
			file.setSize(Bytes.toString(result.getValue(Bytes.toBytes(Constants.FAMILY_FILE_FILE),
					Bytes.toBytes(Constants.COLUMN_FILE_ORIGINALNAMEANDETC[4]))));
			...
			file.setViewflag(Bytes.toString(result.getValue(Bytes.toBytes(Constants.FAMILY_FILE_FILE),
					Bytes.toBytes(Constants.COLUMN_FILE_ORIGINALNAMEANDETC[7]))));
			file.setDate(Bytes.toString(result.getValue(Bytes.toBytes(Constants.FAMILY_FILE_FILE),
					Bytes.toBytes(Constants.COLUMN_FILE_ORIGINALNAMEANDETC[8]))));
		}
		return file;
	}
	...
	public long addFileInfo(File file) {
		long rowKey = hbaseDao.incrCounter(Constants.TABLE_GID, Constants.ROWKEY_GID_FILEID,
				Constants.FAMILY_GID_FILEID, Constants.COLUMN_GID_FILEID, 1);
		hbaseDao.updateOneData(Constants.TABLE_FILE, rowKey, Constants.FAMILY_FILE_FILE,
				Constants.COLUMN_FILE_ORIGINALNAMEANDETC[0], file.getOriginalName());
		hbaseDao.updateOneData(Constants.TABLE_FILE, rowKey, Constants.FAMILY_FILE_FILE,
				Constants.COLUMN_FILE_ORIGINALNAMEANDETC[1], file.getName());
		hbaseDao.updateOneData(Constants.TABLE_FILE, rowKey, Constants.FAMILY_FILE_FILE,
				Constants.COLUMN_FILE_ORIGINALNAMEANDETC[2], file.isFile());
		hbaseDao.updateOneData(Constants.TABLE_FILE, rowKey, Constants.FAMILY_FILE_FILE,
				Constants.COLUMN_FILE_ORIGINALNAMEANDETC[3], file.isDir());
		hbaseDao.updateOneData(Constants.TABLE_FILE, rowKey, Constants.FAMILY_FILE_FILE,
				Constants.COLUMN_FILE_ORIGINALNAMEANDETC[4], file.getSize());
		hbaseDao.updateOneData(Constants.TABLE_FILE, rowKey, Constants.FAMILY_FILE_FILE,
				Constants.COLUMN_FILE_ORIGINALNAMEANDETC[5], file.getPath());
		hbaseDao.updateOneData(Constants.TABLE_FILE, rowKey, Constants.FAMILY_FILE_FILE,
				Constants.COLUMN_FILE_ORIGINALNAMEANDETC[6], file.getOriginalPath());
		hbaseDao.updateOneData(Constants.TABLE_FILE, rowKey, Constants.FAMILY_FILE_FILE,
				Constants.COLUMN_FILE_ORIGINALNAMEANDETC[7], file.getViewflag());
		hbaseDao.updateOneData(Constants.TABLE_FILE, rowKey, Constants.FAMILY_FILE_FILE,
				Constants.COLUMN_FILE_ORIGINALNAMEANDETC[8], file.getDate());
		return rowKey;
	}
	...
	public void addUserFile(User user, long parentid, long rowkey) {
		hbaseDao.updateOneData(Constants.TABLE_USERFILE, user.getId() + "_" + parentid + "_" + rowkey,
				Constants.FAMILY_USERFILE_FILE, Constants.COLUMN_USERFILE_FILEID, rowkey);
	}
	...
	public void renameFileOrFolderInfo(File file, String newname) {
		...
		hbaseDao.updateOneData(Constants.TABLE_FILE, file.getId(), Constants.FAMILY_FILE_FILE,
				Constants.COLUMN_FILE_ORIGINALNAMEANDETC[0], newname);
	}
